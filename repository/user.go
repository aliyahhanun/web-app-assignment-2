...
func (r *userRepository) GetUserTaskCategory() ([]model.UserTaskCategory, error) {
	...
	if err := r.db.Table("users").Select("users.id, users.fullname, users.email, tasks.title AS task, tasks.deadline, tasks.priority, tasks.status, categories.name AS category").
